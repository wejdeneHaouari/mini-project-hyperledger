...
class QueryUtils {
    ...
    */
    async getAssetHistory(reference, owner) {

        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [owner,reference]);
    ...
    async getAll(isHistory, begin,end) {

        
        const resultsIterator = await this.ctx.stub.getStateByRange(begin, end)
